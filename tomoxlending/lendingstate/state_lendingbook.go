...
func (self *lendingExchangeState) CommitInvestingTrie(db Database) error {
	...
	root, err := self.investingTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
...
func (self *lendingExchangeState) CommitBorrowingTrie(db Database) error {
	...
	root, err := self.borrowingTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
		var orderList itemList
		if err := rlp.DecodeBytes(leaf, &orderList); err != nil {
			return nil
		}
...
func (self *lendingExchangeState) CommitLiquidationTimeTrie(db Database) error {
	...
	root, err := self.liquidationTimeTrie.Commit(func(path []byte, leaf []byte, parent common.Hash) error {
		var orderList itemList
		if err := rlp.DecodeBytes(leaf, &orderList); err != nil {
			return nil
		}
