...
func TestTransactionFetcherWaitingWithMeta(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "D", hashes: []common.Hash{{0x01}}, types: []byte{types.LegacyTxType}, sizes: []uint32{999}},
			isWaitingWithMeta(map[string][]announce{
				"A": {
					{common.Hash{0x01}, typeptr(types.LegacyTxType), sizeptr(111)},
					{common.Hash{0x02}, typeptr(types.LegacyTxType), sizeptr(222)},
...
func TestTransactionForgotten(t *testing.T) {
	...
	tx1 := types.NewTransaction(0, common.Address{}, big.NewInt(0), common.DefaultMinGasPrice, big.NewInt(1), nil)
	tx2 := types.NewTransaction(1, common.Address{}, big.NewInt(0), common.DefaultMinGasPrice, big.NewInt(1), nil)
