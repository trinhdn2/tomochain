...
type Peer struct {
	...
	*p2p.Peer                   // The embedded P2P package peer
	...
	knownOrderTxs   *knownCache        // Set of order transaction hashes known to be known by this peer
	knownLendingTxs *knownCache        // Set of lending transaction hashes known to be known by this peer
...
// SendOrderTransactions sends transactions to the peer and includes the hashes
// in its transaction hash set for future reference.
...
// SendLendingTransactions sends transactions to the peer and includes the hashes
// in its transaction hash set for future reference.
...
// MarkOrderTransaction marks a order transaction as known for the peer, ensuring that it
// will never be propagated to this particular peer.
func (p *Peer) MarkOrderTransaction(hash common.Hash) {
	// If we reached the memory allowance, drop a previously known transaction hash
	for p.knownOrderTxs.Cardinality() >= maxKnownOrderTxs {
		p.knownOrderTxs.hashes.Pop()
	}
