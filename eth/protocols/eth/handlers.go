...
const MaxStateFetch = 384 // Amount of node state values to allow fetching per request
...
func handleGetNodeDataMsg(backend Backend, msg Decoder, peer *Peer) error {
	var decodedMsg *p2p.Msg
	// Decode the retrieval message
	err := msg.Decode(decodedMsg)
	if err != nil {
		return err
	}
	msgStream := rlp.NewStream(decodedMsg.Payload, uint64(decodedMsg.Size))
	...
	for bytes < softResponseLimit && len(data) < MaxStateFetch {
		...
		if err := msgStream.Decode(&hash); err == rlp.EOL {
			...
			return fmt.Errorf("msg %v: %v", msg, errDecode)
		}
...
func handleOrderTransactions(backend Backend, msg Decoder, peer *Peer) error {
	...
	var unknownOrderTxs []*types.OrderTransaction

	for i, tx := range txs {
		// Validate and mark the remote transaction
		if tx == nil {
			return fmt.Errorf("%w: order transaction %d is nil", errDecode, i)
		}
		peer.MarkOrderTransaction(tx.Hash())
		exist := peer.knownOrderTxs.Contains(tx.Hash())
		if !exist {
			unknownOrderTxs = append(unknownOrderTxs, tx)
		} else {
			log.Trace("Discard known tx", "hash", tx.Hash(), "nonce", tx.Nonce())
		}
	}
	if peer.orderpool != nil {
		peer.orderpool.AddRemotes(txs)
	}
	return nil
}

func handleLendingTransactions(backend Backend, msg Decoder, peer *Peer) error {
	// Transactions arrived, make sure we have a valid and fresh chain to handle them
	if !backend.AcceptTxs() {
		return nil
	}
	...
	var unknownLendingTxs []*types.LendingTransaction

	for i, tx := range txs {
		// Validate and mark the remote transaction
		if tx == nil {
			return fmt.Errorf("%w: lending transaction %d is nil", errDecode, i)
		}
		peer.MarkLendingTransaction(tx.Hash())
		exist := peer.knownLendingTxs.Contains(tx.Hash())
		if !exist {
			unknownLendingTxs = append(unknownLendingTxs, tx)
		} else {
			log.Trace("Discard known tx", "hash", tx.Hash(), "nonce", tx.Nonce())
		}
	}
	if peer.lendingpool != nil {
		peer.lendingpool.AddRemotes(txs)
	}
	return nil
}
