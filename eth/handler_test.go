...
// SubscribeTxPreEvent should return an event subscription of NewTxsEvent and
...
// pre-initialized with some sane testing defaults and the transaction pool mocked
// out.
...
func newTestHandlerWithBlocks(blocks int) *testHandler {
	...
	handler, err := NewProtocolManager(params.TestChainConfig, downloader.FastSync, 1, new(event.TypeMux), txpool, ethash.NewFaker(), chain, db)
	if err != nil {
		panic(err)
	}
	handler.Start(1000)
